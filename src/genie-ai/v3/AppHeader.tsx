...
export function AppHeader(props: Props) {
  ...
  useEffect(() => {
    if (!isLanguageMenuOpen && !isNavigationMenuOpen) return;
  ...
  }, [isLanguageMenuOpen, isNavigationMenuOpen]);
  ...
  return (
    <header className="z-40 grid h-16 w-full min-w-0 grid-cols-[minmax(0,1fr)_auto] items-center gap-2 border-b border-[#E4E1D8] bg-white px-3 sm:flex sm:justify-between sm:px-5">
      ...
      <div className="flex shrink-0 items-center justify-end gap-1.5 sm:gap-2">
        ...
        <button type="button" onClick={props.onClearHistory} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-[#E4E1D8] text-[#5B6B7A] hover:bg-[#E7EEF7] hover:text-[#1E4D8C] md:w-auto md:gap-1.5 md:rounded-[10px] md:px-3" title={props.clearLabel} aria-label={props.clearLabel}><V3Icon name="trash" className="h-4 w-4" /><span className="hidden text-xs font-semibold md:inline">{props.clearLabel}</span></button>
        ...
        <div ref={navigationMenuRef} className="relative shrink-0">
          <button type="button" aria-label="Open navigation menu" aria-haspopup="menu" aria-expanded={isNavigationMenuOpen} onClick={() => { setIsNavigationMenuOpen((open) => !open); setIsLanguageMenuOpen(false); }} className={`grid h-9 w-9 place-items-center rounded-full border transition ${isNavigationMenuOpen ? "border-[#3D74B8] bg-[#E7EEF7] text-[#1E4D8C]" : "border-[#E4E1D8] text-[#31577F] hover:border-[#9EB7D2] hover:bg-[#F5F8FC]"}`}><V3Icon name="menu" className="h-4 w-4" /></button>
          {isNavigationMenuOpen ? <div role="menu" aria-label="Navigation" className="absolute right-0 top-[calc(100%+8px)] z-[70] grid min-w-36 gap-1 rounded-[14px] border border-[#D7E2EF] bg-white p-1.5 shadow-[0_16px_40px_-18px_rgba(10,31,58,.45)]"><Link href="/features" role="menuitem" onClick={() => setIsNavigationMenuOpen(false)} className="flex h-9 items-center rounded-[9px] px-3 text-xs font-semibold text-[#31577F] transition hover:bg-[#E7EEF7] hover:text-[#123661]">Features</Link><Link href="/demo-video" role="menuitem" onClick={() => setIsNavigationMenuOpen(false)} className="flex h-9 items-center rounded-[9px] px-3 text-xs font-semibold text-[#31577F] transition hover:bg-[#E7EEF7] hover:text-[#123661]">Demo</Link></div> : null}
        </div>
      </div>
    </header>
  );
