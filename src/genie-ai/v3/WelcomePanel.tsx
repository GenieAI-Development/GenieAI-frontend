...
const highlights = [
  ["shopping", "Smart Shopping & Gift box planning", "Live product suggestions & compare"],
  ["gift", "Gift Message & Cards", "Personalized English notes & cards"],
];
    ...
    <div className="fixed inset-0 z-[95] bg-[#071A30]/45 backdrop-blur-sm">
      <section role="dialog" aria-modal="true" aria-labelledby="welcome-title" className="genie-welcome-panel relative w-full overflow-hidden rounded-b-[28px] border-b border-[#D6A936] bg-[#FAF7F1] shadow-[0_24px_70px_rgba(7,26,48,.38)] dark:bg-[#071A30]">
        ...
        <div className="relative mx-auto grid min-h-[430px] w-full max-w-6xl items-center gap-8 px-6 py-12 md:grid-cols-[1.08fr_.92fr] md:px-10 md:py-14">
          <div>
            <div className="flex items-center gap-3"><GenieMark className="h-10 w-10 rounded-xl border border-[#D6A936]" /><p className="genie-wordmark text-[#0B2748] dark:text-white">Genie<span className="text-[#B3872F]">AI</span></p></div>
            <h2 id="welcome-title" className="mt-6 max-w-2xl text-4xl font-semibold leading-[1.08] tracking-[-.04em] text-[#0B2748] dark:text-white md:text-6xl">Welcome To GenieAI</h2>
            <p className="mt-5 max-w-xl text-sm leading-7 text-[#5B6B7A] dark:text-[#AFC8E5] md:text-base">Tell Genie who you are shopping for. It will shape the search, compare the strongest options, and save your message for checkout</p>
            <div className="mt-7 grid grid-cols-2 gap-3 sm:flex"><button type="button" onClick={onClose} className="inline-flex h-11 min-w-0 items-center justify-center gap-1.5 rounded-[11px] bg-[#0B2748] px-2 text-xs font-semibold text-white transition hover:bg-[#123661] sm:px-5 sm:text-sm dark:bg-[#D6A936] dark:text-[#071A30]">Start shopping<V3Icon name="send" className="h-4 w-4 shrink-0" /></button><Link href="/features" className="grid h-11 min-w-0 place-items-center rounded-[11px] border border-[#D7E2EF] bg-white px-2 text-center text-xs font-semibold text-[#31577F] sm:px-5 sm:text-sm dark:border-[#294967] dark:bg-[#102D4D] dark:text-[#AFC8E5]">Explore features</Link></div>
          </div>
          ...
          <div className="rounded-[22px] border border-[#D7E2EF] bg-white/90 p-3 shadow-[0_20px_50px_-32px_rgba(10,31,58,.5)] backdrop-blur dark:border-[#294967] dark:bg-[#102D4D]/90">
            <div className="rounded-2xl bg-[#0B2748] p-5 text-white"><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#D6A936]">One conversation</p><h3 className="mt-2 text-xl font-semibold">From idea to checkout</h3></div>
