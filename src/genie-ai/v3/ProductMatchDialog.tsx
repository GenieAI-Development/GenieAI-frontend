...
type MatchResult = {
  ...
  pairs: Array<{ productAId: string; productBId: string; score: number; matches: boolean; insight: string }>;
    ...
    {open ? <div className="fixed inset-0 z-[100] grid place-items-center p-4">
      ...
      <section role="dialog" aria-modal="true" aria-labelledby="match-insights-title" className="relative z-10 max-h-[85vh] w-full max-w-[620px] overflow-y-auto rounded-[22px] bg-white p-5 text-[#0A1F3A] shadow-[0_24px_70px_rgba(10,31,58,.35)] sm:p-7">
        ...
        {result ? <div className="mt-6 space-y-5">
          ...
          <div className="space-y-3">{result.pairs.map((pair) => <article key={`${pair.productAId}-${pair.productBId}`} className="rounded-xl border border-[#E4E1D8] p-4"><div className="flex items-start justify-between gap-3"><h3 className="text-sm font-semibold leading-5">{names.get(pair.productAId) ?? "Product"} <span className="text-[#9AA7B2]">+</span> {names.get(pair.productBId) ?? "Product"}</h3><span className={`shrink-0 rounded-full px-2.5 py-1 text-[10px] font-bold ${pair.matches ? "bg-[#E7F5EC] text-[#267044]" : "bg-[#FFF0EA] text-[#A64E2E]"}`}>{pair.matches ? "Good match" : "Weak match"} · {pair.score}%</span></div><p className="mt-2 text-xs leading-5 text-[#5B6B7A]">{pair.insight}</p></article>)}</div>
