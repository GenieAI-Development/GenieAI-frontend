...
export function Composer({ children, disabled, formRef, imageInputRef, inputRef, isRecording, onImage, onInput, onFocus, onSubmit, onVoice, placeholder, sendLabel, value }: Props) {
  return <div ref={formRef} className="genie-composer relative shrink-0 bg-[linear-gradient(transparent,#FAF7F1_28%)] px-4 pb-4 pt-3 sm:px-7">
    ...
    <form onSubmit={onSubmit} className="flex items-center gap-1.5 rounded-2xl border border-[#E4E1D8] bg-white p-2 shadow-[0_8px_24px_-12px_rgba(10,31,58,.18)]">
      <button type="button" onClick={onVoice} disabled={disabled} className={`grid h-9 w-9 shrink-0 place-items-center rounded-[10px] transition ${isRecording ? "bg-[#F7E9DF] text-[#B25A2E]" : "text-[#5B6B7A] hover:bg-[#E7EEF7] hover:text-[#1E4D8C]"}`} aria-label="Voice input"><V3Icon name="mic" className="h-[18px] w-[18px]" /></button>
      <button type="button" onClick={() => imageInputRef.current?.click()} disabled={disabled} className="grid h-9 w-9 shrink-0 place-items-center rounded-[10px] text-[#5B6B7A] transition hover:bg-[#E7EEF7] hover:text-[#1E4D8C]" aria-label="Upload a photo"><V3Icon name="camera" className="h-[18px] w-[18px]" /></button>
      ...
      <button type="submit" disabled={disabled || !value.trim()} className="flex h-10 shrink-0 items-center gap-1.5 rounded-[11px] bg-[#0A1F3A] px-4 text-xs font-semibold text-white disabled:cursor-not-allowed disabled:opacity-45 sm:text-sm">{sendLabel}<V3Icon name="send" className="h-4 w-4" /></button>
    </form>
  </div>;
}
