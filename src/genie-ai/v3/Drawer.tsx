...
export function Drawer({ children, icon, open, title, onClose }: { children: ReactNode; icon: string; open: boolean; title: string; onClose: () => void }) {
  if (!open) return null;
  return <>
    <button type="button" aria-label={`Close ${title}`} onClick={onClose} className="fixed inset-0 z-[70] bg-[#0A1F3A]/40 backdrop-blur-[1px]" />
    <aside role="dialog" aria-modal="true" aria-label={title} className="fixed bottom-0 right-0 top-0 z-[80] flex w-full max-w-[400px] flex-col bg-white shadow-[-16px_0_40px_rgba(10,31,58,.22)]">
